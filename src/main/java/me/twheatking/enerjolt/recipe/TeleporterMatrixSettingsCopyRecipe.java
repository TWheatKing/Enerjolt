...
public class TeleporterMatrixSettingsCopyRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingInput container, Level level) {
        ...
        for(int i = 0;i < container.size();i++) {
            ...
            if(!itemStack.isEmpty()) {
                if(!itemStack.is(EnerjoltItems.TELEPORTER_MATRIX.get()))
                    return false;
    ...
    public ItemStack getResultItem(HolderLookup.Provider registries) {
        return new ItemStack(EnerjoltItems.TELEPORTER_MATRIX.get(), 2);
    }
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingInput container) {
        ...
        for(int i = 0; i < remainders.size(); ++i) {
            ...
            if(!itemstack.isEmpty()) {
                ...
                }else if(itemstack.is(EnerjoltItems.TELEPORTER_MATRIX.get()) && TeleporterMatrixItem.isLinked(itemstack)) {
    ...
    public ItemStack getToastSymbol() {
        return new ItemStack(EnerjoltItems.TELEPORTER_MATRIX.get());
    }

    @Override
    public RecipeSerializer<?> getSerializer() {
        return EnerjoltRecipes.TELEPORTER_MATRIX_SETTINGS_COPY_SERIALIZER.get();
    }
